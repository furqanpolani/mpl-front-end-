...
async function getScorecard(id: string) {
  const res = await fetch(`${API}/api/public/fixtures/${id}/scorecard`, { next: { revalidate: 10 } });
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '3rem 1.5rem' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #111118, #16161f)', border: '1px solid #1e1e2e', borderRadius: '16px', padding: '2rem', marginBottom: '2rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            <span className={fixture.status === 'live' ? 'badge-live' : fixture.status === 'upcoming' ? 'badge-upcoming' : 'badge-completed'} style={{ fontSize: '0.8rem', padding: '5px 14px' }}>
              {fixture.status}
            </span>
      ...
      {currentBowlerFigures && (
        <div style={{ background: '#111118', border: '1px solid #c8a84b44', borderRadius: '10px', padding: '1rem 1.5rem', marginBottom: '1.5rem', display: 'flex', gap: '2rem', alignItems: 'center' }}>
          <span style={{ color: '#c8a84b', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em' }}>Current Bowler</span>
          <span style={{ color: '#fff', fontWeight: 600 }}>{currentBowlerFigures.bowlerName}</span>
          <span style={{ color: '#9ca3af', fontSize: '0.875rem' }}>{currentBowlerFigures.oversBowled} ov · {currentBowlerFigures.runsConceded} runs · {currentBowlerFigures.wicketsTaken} wkts</span>
        </div>
      )}
          ...
          {inn.commentary && inn.commentary.length > 0 ? (
            <div style={{ padding: '1rem 1.5rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
                {inn.commentary.slice(0, 20).map((b: any) => (
