...
export default function ArchivePage() {
  const tournaments = mockApi.getArchive();
          ...
          {tournaments.map((t: any, idx: number) => (
            <div key={t.id} style={{ background: '#111118', border: '1px solid #1e1e2e', borderRadius: '16px', overflow: 'hidden' }}>
              <div style={{ height: '200px', position: 'relative', overflow: 'hidden' }}>
                <img src={IMAGES[idx % IMAGES.length]} alt={t.name} style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.5 }} />
              ...
              <div style={{ padding: '1.5rem 2rem' }}>
                <div className="archive-card" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 2fr', gap: '2rem', alignItems: 'start' }}>
                  <div>
                    {t.winner && <div style={{ marginBottom: '1rem' }}><div style={{ color: '#c8a84b', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '4px' }}>🏆 Champion</div><div style={{ color: '#fff', fontWeight: 700 }}>{t.winner.name}</div></div>}
                    {t.runnerUp && <div><div style={{ color: '#6b7280', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '4px' }}>🥈 Runner-up</div><div style={{ color: '#9ca3af', fontWeight: 600 }}>{t.runnerUp.name}</div></div>}
                  </div>
                  <div className="archive-stats" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                    {[['Teams', t.finalPointsTable?.length ?? 0], ['Matches', Math.round(t.finalPointsTable?.reduce((s: number, e: any) => s + e.played, 0) / 2) || 0]].map(([label, val]) => (
                    ...
                    <div>
                      ...
                      <div style={{ borderRadius: '8px', overflow: 'hidden', border: '1px solid #1e1e2e' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
                          <thead><tr style={{ background: '#0d0d14' }}>{['#', 'Team', 'P', 'W', 'Pts'].map(h => <th key={h} style={{ padding: '6px 10px', color: '#6b7280', textAlign: 'left', fontWeight: 600, fontSize: '0.7rem', textTransform: 'uppercase' }}>{h}</th>)}</tr></thead>
                          <tbody>
                ...
                <div style={{ marginTop: '1.25rem' }}>
                  <Link href={`/events/${t.id}`} className="btn-gold" style={{ fontSize: '0.8rem', padding: '8px 20px' }}>View Full Details</Link>
                </div>
              </div>
            </div>
          ))}
