...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    setLiveFixtures(hero.liveFixtures ?? []);
  }, []);

  return (
    <section className="hero-section" style={{ position: 'relative', minHeight: '92vh', display: 'flex', alignItems: 'center', overflow: 'hidden', background: '#0a0a0f' }}>
      <div style={{ position: 'absolute', inset: 0, backgroundImage: `url('https://images.unsplash.com/photo-1540747913346-19e32dc3e97e?w=1920&q=90')`, backgroundSize: 'cover', backgroundPosition: 'center 30%', opacity: 0.35 }} />
      ...
      <div className="hero-content" style={{ maxWidth: '1200px', margin: '0 auto', padding: '5rem 1.5rem 4rem', position: 'relative', width: '100%' }}>
        <div style={{ maxWidth: '680px' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(200,168,75,0.1)', border: '1px solid rgba(200,168,75,0.35)', borderRadius: '4px', padding: '6px 16px', marginBottom: '1.5rem' }}>
            <span style={{ width: '8px', height: '8px', background: liveFixtures.length > 0 ? '#dc2626' : '#c8a84b', borderRadius: '50%', display: 'inline-block', boxShadow: liveFixtures.length > 0 ? '0 0 8px #dc2626' : '0 0 8px #c8a84b' }} />
          ...
          <h1 className="hero-title" style={{ fontFamily: 'Oswald, sans-serif', fontWeight: 700, fontSize: 'clamp(2.2rem, 6vw, 4.5rem)', lineHeight: 1.05, marginBottom: '1rem', textTransform: 'uppercase', letterSpacing: '-0.01em' }}>
            ...
            <span style={{ background: 'linear-gradient(135deg, #c8a84b 0%, #e8c96b 50%, #c8a84b 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', display: 'block' }}>WINNING SYSTEM</span>
          </h1>
          ...
          <div className="hero-buttons" style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '2.5rem' }}>
            ...
            <Link href="/events" style={{ color: '#9ca3af', fontSize: '0.875rem', fontWeight: 600, padding: '12px 0', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '6px' }}>Upcoming Events →</Link>
          </div>
