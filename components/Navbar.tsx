...
export default function Navbar() {
  ...
  const activeTournaments = (mockApi as any).getCricketTournaments?.() ?? sports.flatMap((s: any) => mockApi.getTournamentsBySport(s.id));
      ...
      <div style={{ background: '#c8a84b', padding: '5px 0' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: '0.6rem', alignItems: 'center' }}>
            {[
              { href: 'https://facebook.com', isInsta: false, d: 'M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z' },
              { href: 'https://instagram.com', isInsta: true, d: '' },
              { href: 'https://youtube.com', isInsta: false, d: 'M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46a2.78 2.78 0 0 0-1.95 1.96A29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58A2.78 2.78 0 0 0 3.41 19.6C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 0 0 1.95-1.95A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z' },
              { href: 'https://twitter.com', isInsta: false, d: 'M23 3a10.9 10.9 0 0 1-3.14 1.53 4.48 4.48 0 0 0-7.86 3v1A10.66 10.66 0 0 1 3 4s-4 9 5 13a11.64 11.64 0 0 1-7 2c9 5 20 0 20-11.5a4.5 4.5 0 0 0-.08-.83A7.72 7.72 0 0 0 23 3z' },
            ].map((s, i) => (
              <a key={i} href={s.href} target="_blank" rel="noreferrer" style={{ width: '22px', height: '22px', background: 'rgba(0,0,0,0.15)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#000' }}>
                {s.isInsta ? <svg width="11" height="11" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="2" y="2" width="20" height="20" rx="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', alignItems: 'center', height: '60px', justifyContent: 'space-between' }}>
          ...
          <div className="desktop-nav" style={{ display: 'flex', alignItems: 'center', gap: '0', flex: 1, justifyContent: 'center' }}>
            ...
            <div style={{ position: 'relative' }}>
              <button onClick={() => setTournamentOpen(o => !o)} style={{ background: 'none', border: 'none', color: tournamentOpen ? '#c8a84b' : '#9ca3af', fontSize: '0.8rem', fontWeight: 600, padding: '8px 12px', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'flex', alignItems: 'center', gap: '4px' }}>
              ...
              {tournamentOpen && (
                <div style={{ position: 'absolute', top: '100%', left: 0, background: '#111118', border: '1px solid #1e1e2e', borderRadius: '8px', minWidth: '200px', boxShadow: '0 8px 32px rgba(0,0,0,0.6)', zIndex: 200, overflow: 'hidden' }}>
                  <div style={{ padding: '0.5rem 0' }}>
                    ...
                    {activeTournaments.filter((t: any) => t.status === 'active').map((t: any) => (
            ...
            {sports.map((s: any) => <NavLink key={s.id} href={`/sports/${s.slug}`} onClick={closeAll}>{s.name}</NavLink>)}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: '#0d0d14', zIndex: 999, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
          ...
          <nav style={{ padding: '1rem 1.5rem', flex: 1 }}>
            {[['/', 'Home'], ['/sports/cricket', '🏏 Cricket'], ['/sports/paddle', '🏓 Paddle'], ['/announcements', '📰 News'], ['/events', '📅 Events'], ['/archive', '🏆 Archive'], ['/marketplace', '💰 Marketplace'], ['/about', 'ℹ About']].map(([href, label]) => (
              <Link key={href} href={href} onClick={closeAll} style={{ display: 'block', color: '#fff', fontSize: '1.1rem', fontWeight: 600, padding: '1rem 0', borderBottom: '1px solid #1e1e2e', textDecoration: 'none', textTransform: 'uppercase', letterSpacing: '0.05em', fontFamily: 'Oswald, sans-serif' }}>
                {label}
              </Link>
            ))}
            <button onClick={() => setMobileTournamentOpen(o => !o)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', color: '#fff', fontSize: '1.1rem', fontWeight: 600, padding: '1rem 0', background: 'none', border: 'none', borderBottom: '1px solid #1e1e2e', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.05em', fontFamily: 'Oswald, sans-serif' }}>
            ...
            {mobileTournamentOpen && (
              <div style={{ paddingLeft: '1rem', borderBottom: '1px solid #1e1e2e' }}>
                {activeTournaments.map((t: any) => (
                  <Link key={t.id} href={`/tournaments/${t.id}`} onClick={closeAll} style={{ display: 'block', color: '#c8a84b', fontSize: '0.95rem', padding: '0.75rem 0', textDecoration: 'none', borderBottom: '1px solid #0f0f18' }}>
                    {t.name}
                  </Link>
                ))}
              </div>
...
function NavLink({ href, children, onClick }: { href: string; children: React.ReactNode; onClick?: () => void }) {
  return (
    <Link href={href} onClick={onClick} style={{ color: '#9ca3af', fontSize: '0.8rem', fontWeight: 600, padding: '8px 10px', textDecoration: 'none', textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}
...
function DropLink({ href, children, onClick }: { href: string; children: React.ReactNode; onClick?: () => void }) {
  return (
    <Link href={href} onClick={onClick} style={{ display: 'block', padding: '8px 1rem', color: '#9ca3af', fontSize: '0.85rem', fontWeight: 600, textDecoration: 'none', textTransform: 'uppercase', letterSpacing: '0.05em' }}
